...
public class ${upperDataKey}Controller {
    ...
    public BaseResponse<Long> add${upperDataKey}(@RequestBody ${upperDataKey}AddRequest ${dataKey}AddRequest,
        ...
        if (${dataKey}.getIsBuiltin() == null) {
            ${dataKey}.setIsBuiltin(0);
        }
        if (${dataKey}.getIsSuperAdmin() == null) {
            ${dataKey}.setIsSuperAdmin(0);
        }
        if (${dataKey}.getDataScope() == null) {
            ${dataKey}.setDataScope(1);
        }
    ...
    public BaseResponse<Boolean> delete${upperDataKey}(@PathVariable("id") long id) {
        ...
        Role old${upperDataKey} = ${dataKey}Service.getById(id);
        ThrowUtils.throwIf(old${upperDataKey} == null, ErrorCode.NOT_FOUND_ERROR);
        ThrowUtils.throwIf(old${upperDataKey}.getIsBuiltin() != null && old${upperDataKey}.getIsBuiltin() == 1,
    ...
    public BaseResponse<Boolean> update${upperDataKey}(@RequestBody ${upperDataKey}UpdateRequest ${dataKey}UpdateRequest) {
        ...
        Role old${upperDataKey} = ${dataKey}Service.getById(${dataKey}UpdateRequest.getId());
        ThrowUtils.throwIf(old${upperDataKey} == null, ErrorCode.NOT_FOUND_ERROR);
        Role ${dataKey} = new Role();
    ...
    public BaseResponse<${upperDataKey}VO> get${upperDataKey}VOById(long id) {
        ...
        Role ${dataKey} = ${dataKey}Service.getById(id);
    ...
    @AuthCheck(mustRole = UserConstant.ADMIN_ROLE)
    public BaseResponse<Page<Role>> list${upperDataKey}ByPage(@RequestBody ${upperDataKey}QueryRequest ${dataKey}QueryRequest) {
        long current = ${dataKey}QueryRequest.getCurrent();
        long size = ${dataKey}QueryRequest.getPageSize();
        Page<Role> ${dataKey}Page = ${dataKey}Service.page(new Page<>(current, size),
